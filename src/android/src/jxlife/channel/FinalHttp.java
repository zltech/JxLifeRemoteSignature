...
import afinal.http.RetryHandler;
import afinal.http.SyncRequestHandler;
...
public class FinalHttp {
    ...
    private static int socketTimeout = 120000; //超时时间，默认10秒
    private static int maxRetries = 0;//错误尝试次数，错误异常表请在RetryHandler添加
  ...
  //------------------put 请求-----------------------
    ...
    protected <T> void sendRequest(DefaultHttpClient client, HttpContext httpContext, HttpUriRequest uriRequest, String contentType, AjaxCallBack<T> ajaxCallBack) {
        ...
        handler = new HttpHandler<T>(client, httpContext, ajaxCallBack,charset);
        handler.executeOnExecutor(executor, uriRequest);
